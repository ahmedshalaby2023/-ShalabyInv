...
FG_OFFICIAL_HOLIDAY_FILE = Path("fg_official_holidays.json")
...
def _load_official_holiday_dates() -> set[date]:
    if not FG_OFFICIAL_HOLIDAY_FILE.exists():
        return set()
    try:
        payload = json.loads(FG_OFFICIAL_HOLIDAY_FILE.read_text(encoding="utf-8"))
    except Exception:
        return set()

    if isinstance(payload, dict):
        if "dates" in payload:
            candidates = payload.get("dates", [])
        elif "holidays" in payload:
            candidates = payload.get("holidays", [])
        else:
    ...
    for entry in candidates:
        ts = pd.to_datetime(entry, errors="coerce")
        if pd.isna(ts):
            continue
...
def _count_working_days(
    ...
    *,
    ...
    day_range = pd.date_range(start=start, end=end - pd.Timedelta(days=1), freq="D")

    return sum(
        1
        for day in day_range
        if day.weekday() not in excluded and day.date() not in holidays
    )
...
def render_fg_explorer(materials_df: pd.DataFrame | None = None, bom_bytes: bytes | None = None) -> None:
    ...
    def _prefixed_metric_candidates(candidate: str, prefix: str) -> list[str]:
        ...
        if lowered.startswith("cur"):
            suffix = normalized[3:]
            variants.append(f"{prefix_clean}{suffix}")
            variants.append(f"{prefix_clean.upper()}{suffix}")
        elif lowered.startswith("next"):
            suffix = normalized[4:]
            variants.append(f"{prefix_clean}{suffix}")
            variants.append(f"{prefix_clean.upper()}{suffix}")
        else:
            variants.append(f"{prefix_clean}{normalized}")
            variants.append(f"{prefix_clean.upper()}{normalized}")
        ...
        for variant in variants:
            if variant not in seen:
                ...
                seen.add(variant)
        return ordered_variants or [normalized]

    def _build_metric_column_map(columns: pd.Index, prefix: str) -> dict[str, str | None]:
        mapping: dict[str, str | None] = {}
        for metric, candidates in FG_METRIC_CANDIDATES.items():
            search_candidates: list[str] = []
            if prefix.lower() != "cur":
                for candidate in candidates:
                    for variant in _prefixed_metric_candidates(candidate, prefix):
                        if variant not in search_candidates:
                            search_candidates.append(variant)
            for candidate in candidates:
                if candidate not in search_candidates:
                    search_candidates.append(candidate)
            mapping[metric] = _find_column(columns, search_candidates)
        return mapping

    def _format_metric_display(metric_key: str, prefix: str) -> str:
        if prefix.lower() == "cur" or not prefix:
            return metric_key
        if metric_key.startswith("Cur"):
            return f"{prefix}{metric_key[3:]}"
        if metric_key.lower().startswith("next"):
            return f"{prefix}{metric_key[4:]}"
        if metric_key == "Oh":
            return f"{prefix}OH"
    ...
    month_prefix = "Cur" if month_view == "Current Month" else "Prv"

    metric_column_map = _build_metric_column_map(fg_df.columns, month_prefix)
    ...
    st.markdown(f"### 📊 {month_view} Metrics Overview")
    metric_sequence: list[str] = []
    metric_totals: list[float] = []
    missing_metrics: list[str] = []
    pipe_indicator: float | None = None

    def _metric_total(metric: str) -> float | None:
        column = metric_column_map.get(metric)
        if column and column in filtered_df.columns:
            series = pd.to_numeric(filtered_df[column], errors="coerce").fillna(0)
    ...
    for metric in FG_METRIC_ORDER:
        ...
        if column_name and column_name in filtered_df.columns:
            numeric_series = pd.to_numeric(filtered_df[column_name], errors="coerce").fillna(0)
            metric_sequence.append(metric)
            metric_totals.append(float(numeric_series.sum()))
        else:
            missing_metrics.append(metric)

    cur_os_total = _metric_total("CurOS") or 0.0
    cur_ap_total = _metric_total("CurAP") or 0.0
    cur_as_total = _metric_total("CurAS") or 0.0
    cur_foc_total = _metric_total("CurFOC") or 0.0
    oh_total = _metric_total("Oh") or 0.0
    cur_st_total = _metric_total("CurST") or 0.0

    if cur_os_total + cur_ap_total > cur_as_total + cur_foc_total + oh_total:
        pipe_indicator = (cur_os_total + cur_ap_total) - (cur_as_total + cur_foc_total + oh_total)

    today = pd.Timestamp.today().normalize()
    if month_prefix.lower() == "cur":
        month_start = today.replace(day=1)
        month_end = month_start + relativedelta(months=1)
        elapsed_end = today + pd.Timedelta(days=1)
    else:
        current_month_start = today.replace(day=1)
        month_start = current_month_start - relativedelta(months=1)
        month_end = current_month_start
        elapsed_end = month_end

    official_holidays = _load_official_holiday_dates()
    excluded_weekdays = {4}  # Friday only

    total_workdays = _count_working_days(
        month_start,
        month_end,
        holiday_dates=official_holidays,
        excluded_weekdays=excluded_weekdays,
    )
    elapsed_workdays = _count_working_days(
        month_start,
        elapsed_end,
        holiday_dates=official_holidays,
        excluded_weekdays=excluded_weekdays,
    ...
    target_daily_sales = (cur_st_total / total_workdays) if total_workdays else None
    actual_daily_sales = (cur_as_total / elapsed_workdays) if elapsed_workdays else None

    pace_pct = (
        (actual_daily_sales / target_daily_sales) * 100
        if target_daily_sales and actual_daily_sales
        else None
    )
    pace_delta = (pace_pct - 100) if pace_pct is not None else None

    status_label = "📊 Data pending"
    bg_style = "linear-gradient(135deg, #f0f3f4 0%, #d5dbdb 100%)"
    text_color = "#1c2833"
    if pace_pct is not None:
        if pace_pct >= 105:
            status_label = "🌟 Ahead of plan"
            bg_style = "linear-gradient(135deg, #0f5132 0%, #1abc9c 90%)"
            text_color = "#fefefe"
        elif pace_pct >= 95:
            status_label = "⚖️ On track"
            bg_style = "linear-gradient(135deg, #7f6000 0%, #f1c40f 90%)"
            text_color = "#1c2833"
        else:
            status_label = "🚨 Behind plan"
            bg_style = "linear-gradient(135deg, #641e16 0%, #e74c3c 90%)"
            text_color = "#fdfefe"

    total_text = f"{total_workdays}" if total_workdays else "—"
    elapsed_text = f"{elapsed_workdays}" if elapsed_workdays else "—"
    target_text = f"{target_daily_sales:,.0f}" if target_daily_sales is not None else "—"
    actual_text = f"{actual_daily_sales:,.0f}" if actual_daily_sales is not None else "—"
    pace_text = f"{pace_pct:,.1f}%" if pace_pct is not None else "—"
    delta_text = f"{pace_delta:,.1f} pp" if pace_delta is not None else "—"

    st.markdown(f"### 🚦 Sales Pace Monitor — {month_view}")
    pace_box = f"""
        <div style="border-radius:18px; padding:1.4rem 1.6rem; background:{bg_style}; color:{text_color}; box-shadow:0 12px 30px rgba(0,0,0,0.12);">
            <div style="display:flex; flex-direction:column; gap:0.6rem;">
                <div style="font-size:1.4rem; font-weight:700; letter-spacing:0.5px;">{status_label}</div>
                <div style="display:flex; flex-wrap:wrap; gap:1.2rem; font-size:1.1rem;">
                    <span>🏢 Working days: <strong>{total_text}</strong></span>
                    <span>🕒 Elapsed: <strong>{elapsed_text}</strong></span>
                    <span>🎯 Target/day: <strong>{target_text}</strong></span>
                    <span>🏁 Actual/day: <strong>{actual_text}</strong></span>
                    <span>📈 Pace: <strong>{pace_text}</strong></span>
                    <span>Δ vs plan: <strong>{delta_text}</strong></span>
                </div>
            </div>
        </div>
    """
    ...
    st.caption(
        "Counts working days (Fri & official holidays excluded) for the selected month; target spreads CurST across those days, actual uses CurAS delivered within the same window."
    )

    if metric_sequence:
        metric_labels = [_format_metric_display(metric, month_prefix) for metric in metric_sequence]
        metric_colors = [FG_METRIC_COLOR_MAP.get(metric, "#7f8c8d") for metric in metric_sequence]
        metric_text = [f"{value:,.0f}" for value in metric_totals]

        fig_monthly_metrics = go.Figure(
            data=[
                go.Bar(
                    x=metric_labels,
                    y=metric_totals,
        ...
        if pipe_indicator and pipe_indicator > 0:
            max_value = max(metric_totals) if metric_totals else 0
            fig_monthly_metrics.add_annotation(
                x="CurAP",
                y=max_value,
                text=f"🚛 Pipe Sales ~ {pipe_indicator:,.0f}",
                showarrow=False,
                font={"size": 16, "color": "#d35400", "weight": "bold"},
                bgcolor="rgba(255, 237, 188, 0.85)",
                bordercolor="#d35400",
                borderwidth=1,
                borderpad=6,
                xanchor="center",
        ...
        st.plotly_chart(fig_monthly_metrics, use_container_width=True)
    else:
        st.info("Monthly metric columns not available in the current dataset.")

    if missing_metrics and len(missing_metrics) < len(FG_METRIC_ORDER):
        missing_labels = ", ".join(FG_METRIC_LABELS.get(metric, metric) for metric in missing_metrics)
        st.caption(f"ℹ️ Metrics not found in data: {missing_labels}")
...
def _get_ai_client() -> "OpenAI | None":
    ...
    env_api_key = os.getenv("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")
